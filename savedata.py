...
# ================== 第二部分：投影矩阵计算 ==================
def project_points(transformed_points, K, camera_pose):
    """投影函数（与原始代码保持一致）"""
    R = camera_pose[:3, :3]
    t = camera_pose[:3, 3]
...
if __name__ == '__main__':
    ...
    while(jishuqi <= 1000):
        ...
        # 投影矩阵计算
        projected_points = project_points(transformed_points, K, np.linalg.inv(camera_pose))
        ...
        # 保存数据
        save_data = np.array([mean_error_EPnP, mean_error_P3P])
        if jishuqi != 1:
            with open('./Result/mean_error.txt', 'a') as f:
                np.savetxt(f, save_data, fmt='%.4f', newline=' ')
                f.write('\n')
        else:
